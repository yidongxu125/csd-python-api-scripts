...
# 2026-08-08: Modified by user to support multithreading (ThreadPoolExecutor)
# 2026-08-11: Modified by user to use API filename instead of molecule identifier for folder naming
#
"""
...
def make_pair_file(api_molecule, tempdir, f, api_base_name):
    """
    Creates a file for the api/coformer pair
    Args:
        api_molecule: API molecule object
        tempdir: temporary directory
        f: coformer file path
        api_base_name: base name of the API file (without extension)
    """
    with io.MoleculeReader(f) as reader:
        coformer_molecule = reader[0]
        coformer_name = coformer_molecule.identifier
        molecule_pair = make_molecule_pair(api_molecule, coformer_molecule, api_base_name)
...
def make_molecule_pair(api_molecule, coformer_molecule, api_base_name):
    """
    Creates the multi-component system for each api/coformer pair
    Args:
        api_molecule: API molecule object
        coformer_molecule: coformer molecule object
        api_base_name: base name of the API file (without extension)
    """
    # Use the API filename as the identifier instead of the molecule's internal identifier
    new_file_name = '%s--%s' % (api_base_name, coformer_molecule.identifier)
...
def process_single_coformer(f, api_molecule, api_base_name, work_directory, ignore_intra):
    """
    处理单个 coformer 的函数，用于多线程调用
    
    Args:
        f (str): coformer 文件路径
        api_molecule: API 分子对象
        api_base_name (str): API 文件名（不含扩展名）
        work_directory (str): 工作目录
        ignore_intra (bool): 是否忽略分子内氢键
    
    Returns:
        tuple: (coformer_name, result, error_message)
    """
    try:
        # 为每个线程创建独立的临时目录
        tempdir = tempfile.mkdtemp()
        
        # 创建 API/Coformer 分子对文件，传入 API 文件名
        molecule_file, coformer_name = make_pair_file(api_molecule, tempdir, f, api_base_name)
        print(f"[线程 {threading.current_thread().name}] 处理: {coformer_name}")
        
        # 读取晶体结构
        crystal_reader = io.CrystalReader(molecule_file)
        crystal = crystal_reader[0]
        
        # 创建工作目录 - 现在文件夹名会使用 API 文件名而不是分子 identifier
        ...
        if os.path.exists(os.path.join(directory, "success.json")):
            ...
            return coformer_name, result, None
        ...
        # 计算 MC 分数
        result = get_mc_scores(propensities, crystal.identifier, ignore_intra)
        
        # 保存成功结果
        with open(os.path.join(directory, "success.json"), "w") as file:
            json.dump(result, file)
        
        print(f"[线程 {threading.current_thread().name}] 完成: {coformer_name}")
        return coformer_name, result, None
        
    except Exception as error_message:
        print(f"[线程 {threading.current_thread().name}] 失败: {coformer_name} - {error_message}")
        error_string = f"{coformer_name}: {error_message}"
        return coformer_name, ["N/A", "N/A", "N/A", "N/A", "N/A", "N/A"], error_string


def main(structure, work_directory, failure_directory, library, csdrefcode, ignore_intra, force_run, num_threads):
    ...
    if csdrefcode:
        ...
        # 对于 CSD refcode，使用 refcode 作为 API 名称
        api_base_name = structure
    else:
        crystal = io.CrystalReader(structure)[0]
        # 从文件路径中提取文件名（不含扩展名）作为 API 名称
        api_base_name = os.path.splitext(os.path.basename(structure))[0]
    ...
    if len(coformer_files) == 0:
        print(f'Error! No .mol2 files found in {library}')
        quit()
    
    print(f"找到 {len(coformer_files)} 个 coformer 文件")
    print(f"API 将使用文件名: {api_base_name}")
    print(f"使用 {num_threads} 个线程并行计算...")
    
    mc_dictionary = {}
    failures = []
    
    # 使用多线程执行计算（ThreadPoolExecutor 不需要 pickle SWIG 对象）
    process_func = partial(
        process_single_coformer,
        api_molecule=api_molecule,
        api_base_name=api_base_name,  # 传入 API 文件名
        work_directory=work_directory,
        ignore_intra=ignore_intra
    )
    
    # 使用 ThreadPoolExecutor 进行多线程计算
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        # 提交所有任务
        futures = {
            executor.submit(process_func, f): f for f in coformer_files
        }
        
        # 收集结果
        completed = 0
        for future in as_completed(futures):
            coformer_name, result, error = future.result()
            completed += 1
            print(f"进度: {completed}/{len(coformer_files)}")
            
            if error:
                failures.append(error)
                mc_dictionary[coformer_name] = result
            else:
                mc_dictionary[coformer_name] = result
    ...
    # 过滤掉失败的结果进行排序
    valid_results = {k: v for k, v in mc_dictionary.items() if v[0] != 'N/A'}
    failed_results = {k: v for k, v in mc_dictionary.items() if v[0] == 'N/A'}
    
    mc_hbp_screen = sorted(valid_results.items(), key=lambda e: e[1][0], reverse=True)
    # 将失败的结果放在最后
    for k, v in failed_results.items():
        mc_hbp_screen.append((k, v))
    ...
    # 打印统计信息
    print(f"\n=== 计算统计 ===")
    print(f"总共: {len(coformer_files)} 个 coformer")
    print(f"成功: {len(valid_results)} 个")
    print(f"失败: {len(failed_results)} 个")
    if failures:
        print(f"失败详情请查看: {os.path.join(failure_directory, 'failures.txt')}")
...
if __name__ == '__main__':
    ...
    default_workers = max(1, cpu_count - 1)  # 默认使用 CPU 核心数-1
    ...
    parser.add_argument('-p', '--processes', type=int, default=default_workers,
                        help=f'Number of threads to use for parallel computation (default: {default_workers}, CPU cores: {cpu_count})')
    ...
    if args.processes < 1:
        args.processes = 1
    elif args.processes > cpu_count * 2:
        print(f"警告: 指定的线程数 ({args.processes}) 过多，将使用 {cpu_count * 2}")
    ...
    main(args.input_structure, args.directory, args.failure_directory, args.coformer_library, refcode,
         args.ignore_intra, args.force_run_disordered, args.processes)